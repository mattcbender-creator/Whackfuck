...
export type FaceType = 'angry' | 'laugh' | 'fire' | 'smug';
...
const GENERIC: Array<{ text: (p: string[]) => string; face: FaceType }> = [
  ...
  { face: 'laugh', text: p => `The groundskeeping crew filed a trauma report after watching ${them(p)} take that divot.` },
  ...
  { face: 'angry', text: p => `The cart girl drove past ${them(p)} without stopping. She's seen the scorecard.` },
  ...
  { face: 'laugh', text: p => `The golf ball didn't go out of bounds. It was fleeing ${them(p)}.` },
  ...
  { face: 'laugh', text: p => `${youPl(p).charAt(0).toUpperCase() + youPl(p).slice(1)}'ve done more damage to this course than the frost last winter.` },
  ...
  { face: 'angry', text: p => `${youPl(p).charAt(0).toUpperCase() + youPl(p).slice(1)}'re playing this like it owes ${youPl(p)} money. It doesn't.` },
  { face: 'laugh', text: p => `A 9-year-old at a birthday golf party just outdrew ${them(p)}. Twice.` },
  ...
  { face: 'angry', text: p => `Water hazards see ${them(p)} coming and get excited.` },
  { face: 'laugh', text: p => `Three-putting is a choice. A bad one. But it's ${plural(p, 'your', 'both of yours', 'all of yours')}.` },
  { face: 'laugh', text: p => `${youPl(p).charAt(0).toUpperCase() + youPl(p).slice(1)}'d play better blindfolded. Worth trying.` },
...
const OVER_PAR: Array<{ text: (p: string[]) => string; face: FaceType }> = [
  ...
  { face: 'angry', text: p => `${youPl(p).charAt(0).toUpperCase() + youPl(p).slice(1)} play this badly for free. Imagine if someone paid ${youPl(p)} to suck.` },
...
const EVEN: Array<{ text: (p: string[]) => string; face: FaceType }> = [
  ...
  { face: 'angry', text: p => `Even par. ${youPl(p).charAt(0).toUpperCase() + youPl(p).slice(1)} exist. Congratulations on the bare minimum.` },
...
const PLAYER_TEMPLATES: PlayerEntry[] = [
  ...
  { face: 'laugh', text: (n) => `${n} asked the cart girl for help reading the green. She laughed and drove away.` },
  { face: 'angry', text: (n) => `${n} plays every hole like it's a new traumatic experience.` },
  ...
  { face: 'laugh', text: (n) => `${n} is what happens when confidence doesn't come with receipts.` },
  ...
  { face: 'angry', text: (n) => `${n} missed a two-footer. Closer to the hole than your dad was to being there for you.` },
  { face: 'laugh', text: (n) => `${n} lined up that putt, looked confident, and fucked it completely. Classic ${n}.` },
  ...
  { face: 'angry', text: (n) => `${n} swings that club like they fucking hate it. The club hates them back.` },
  { face: 'laugh', text: (n) => `${n} just out-sliced a banana. On a par 3.` },
  { face: 'laugh', text: (n) => `${n} needs a GPS to find the fairway and still ends up in the trees.` },
];
...
const TEAM_TEMPLATES: TeamEntry[] = [
  ...
  { face: 'laugh', text: (t) => `${t} came to play golf. Golf did not consent.` },
  { face: 'fire', text: (t, p) => `Whack Fuck Cup regrets inviting ${t}. ${fags(p).charAt(0).toUpperCase() + fags(p).slice(1)} are fucking useless.` },
  { face: 'angry', text: (t) => `${t}: talked absolute shit in the parking lot, completely different story out here.` },
  ...
  { face: 'angry', text: (t, p) => `${t} as a unit produces less golf than a blind dog with one leg. Remarkable ${youPl(p)}.` },
  { face: 'angry', text: (t, p) => `${t} sent ${fags(p)} out here and somehow none of you can hit a straight ball.` },
  { face: 'laugh', text: (t, p) => `The ${t} boys are out here looking like ${plural(p, 'he\'s', 'they\'ve', 'they\'ve')} never seen grass before.` },
];
...
const HIGH_SCORE: HighScoreEntry[] = [
  ...
  { face: 'laugh', text: (p, _par, s) => `${s} strokes. That number should require a fucking permit.` },
  { face: 'fire', text: (p, par, s) => `${s - par} over par on one hole, ${youPl(p)}. One. Hole.` },
  { face: 'laugh', text: (p) => `${fags(p).charAt(0).toUpperCase() + fags(p).slice(1)}, that snowman is the most activity any of you will get with a ball this year.` },
...
const HOLE_SPECIFIC: Record<number, { text: string; face: FaceType }> = {
  1:  { face: 'laugh', text: 'First hole and already looking like that. Not a great omen.' },
  ...
  6:  { face: 'laugh', text: 'Hole 6 is forgiving. What just happened on the last one was not forgivable.' },
  ...
  11: { face: 'angry',     text: 'Hole 11 is when people pretend the front nine didn\'t happen. It happened.' },
  ...
  17: { face: 'laugh', text: 'Second to last. Just enough time to make it worse. And you might.' },
...
const SCORE_CONTEXT: Array<{ text: (p: string[]) => string; face: FaceType }> = [
  ...
  { face: 'laugh', text: p => `The Whack Fuck Cup has seen some bad golf. ${fags(p).charAt(0).toUpperCase() + fags(p).slice(1)} are genuinely remarkable even by those standards.` },
  { face: 'angry', text: p => `Somewhere ${plural(p, 'your', 'both your', 'all your')} old golf coach${plural(p, ' is', 'es are', 'es are')} having a drink and not thinking about ${youPl(p)}. Good for ${plural(p, 'him', 'them', 'them')}.` },
